...
import { Row, Col, Card, Typography, Button, Tag, Space, Progress, Avatar } from 'antd';
import {
...
const ClientDashboard: React.FC = () => {
  ...
  // Data Scope Restriction: Strictly TechCorp Việt Nam jobs only
  const techCorpJobs = MOCK_JOBS.filter(
  ...
  return (
    <div>
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {[
          { label: 'Vị trí TechCorp đang tuyển', value: techCorpJobs.filter(j => j.status === 'ACTIVE').length, icon: <FileTextOutlined />, color: '#0284c7', action: () => navigate('/client/jobs') },
          { label: 'Tổng số ứng viên nộp', value: techCorpJobs.reduce((a, j) => a + j.applicationCount, 0), icon: <TeamOutlined />, color: '#8b5cf6', action: () => navigate('/client/candidates') },
          { label: 'Đã duyệt phỏng vấn', value: techCorpJobs.reduce((a, j) => a + j.shortlistedCount, 0), icon: <TrophyOutlined />, color: '#10b981', action: () => navigate('/client/interviews-offers') },
          { label: 'Theo dõi bảo hành (COD)', value: 3, icon: <ClockCircleOutlined />, color: '#f59e0b', action: () => navigate('/client/warranty') },
        ].map((stat) => (
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={14}>
          <Card
            title={<span style={{ fontWeight: 700 }}>Tin tuyển dụng của TechCorp Việt Nam</span>}
            style={{ borderRadius: 14 }}
            extra={<Button type="link" size="small" onClick={() => navigate('/client/jobs')}>Xem tất cả</Button>}
          >
            {techCorpJobs.map(job => (
...
const AffiliateDashboard: React.FC = () => {
  const navigate = useNavigate();
  const summary = MOCK_LEDGER_SUMMARY;
  return (
    <div>
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {[
...
const HRDashboard: React.FC = () => {
  ...
  return (
    <div>
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {[
          { label: 'Vị trí cần sàng lọc', value: 4, color: '#ef4444' },
          { label: 'Hồ sơ chờ duyệt', value: MOCK_CANDIDATES.length, color: '#0284c7' },
          { label: 'Phỏng vấn trong tuần', value: 3, color: '#10b981' },
          { label: 'Đề nghị tuyển dụng (Offer)', value: 2, color: '#f59e0b' },
        ].map((s) => (
    ...
    <div>
      ...
      {role === UserRole.CANDIDATE && (
        <Card style={{ borderRadius: 14, textAlign: 'center', padding: 40 }}>
          ...
          <Title level={3}>Trung tâm tìm kiếm việc làm</Title>
          <Text type="secondary">Khám phá các vị trí tuyển dụng hấp dẫn, tạo CV chuyên nghiệp và theo dõi tiến độ ứng tuyển.</Text>
          <div style={{ marginTop: 20 }}>
            <Space>
              <Button type="primary" onClick={() => navigate('/jobs')} style={{ borderRadius: 8 }}>Khám phá việc làm</Button>
              <Button onClick={() => navigate('/cv-builder')} style={{ borderRadius: 8 }}>Tạo CV chuyên nghiệp</Button>
            </Space>
          </div>
        </Card>
      )}
